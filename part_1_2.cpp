...
- Implementation theory: Use a stack to find every matching pair of brackets. For each pair, determine whether it contains purely a product,
or if it contains a sum. Once each pair is determined, loop through the input, and whenever we reach a "warning" input (a multiplication followed by something unknown)
then we check that there is no violation (sum * product, multi-var NOT). If we find a "+", we are fine, if not, we must check. I will use a vector of a pair of a pair of integers
...
I need to make sure that the bracket does not contain more than one variable, for example, (abc)'.
*/
